...
def generate_latent_noise(b_size=batch_size, n_latents=nz, clip_value=None, supress_mix_style=False):
    out1 = [torch.randn(b_size, n_latents, device=device)] if current_step < step_mix_style else [torch.randn(b_size, nz, device=device) for _ in range(2)]
